...
import {
  createFtpSocksSocket,
  FtpClient,
  patchFtpsUploadSocketEnd,
  type FtpBackend,
} from "../src/clients/ftp.ts";
...
class FailingAccessFtpBackend extends FakeFtpBackend {
  override async access(
    options: Parameters<FtpBackend["access"]>[0],
  ): Promise<void> {
    this.accessCalls.push(options);
...
describe("FtpClient", () => {
  test("creates FTP sockets that connect through SOCKS5", async () => {
    ...
    const calls: unknown[] = [];
    const socket = createFtpSocksSocket(
    ...
    expect(inner.writes.map((chunk) => chunk.toString("utf8"))).toEqual([
      "USER anonymous\r\n",
    ]);
  ...
  test("waits for the SOCKS5 socket to finish ending before reporting upload data completion", async () => {
    ...
    const socket = createFtpSocksSocket(
      { host: "proxy.example.com", port: 1080 },
      async () => inner as unknown as Socket,
    );
    let ended = false;
  ...
  test("uses Bun TLS shutdown for encrypted upload data sockets", async () => {
    ...
    expect(socket.writes.map((chunk) => chunk.toString("utf8"))).toEqual([
      "payload",
    ]);
  ...
  test("downloads, uploads, deletes, creates directories, and closes", async () => {
    ...
    expect(backend.downloadCalls).toEqual([
      { localPath: localDownload, remotePath: "/remote/source.txt" },
    ]);
    expect(backend.uploadCalls).toEqual([
  ...
  test("wraps lazy connection failures in operation errors", async () => {
    ...
    expect(downloadError.message).toBe(
      "Failed to download '/remote/source.txt' from FTP host 'ftp.example.com': connect failed",
    );
    expect(uploadError).toBeInstanceOf(TransferError);
    expect(uploadError.message).toBe(
      `Failed to upload '${localUpload}' to FTP host 'ftp.example.com': connect failed`,
    );
  });
