...
describe("cli", () => {
  test("lists configured remotes", async () => {
    ...
    const code = await main(["--config", configPath, "remotes"], {
      stdout,
      stderr,
    });
  ...
  test("creates and reports the first-run default config path", async () => {
    ...
    expect(stderr.value).toContain(
      `Created default configuration at ${defaultConfigPath}`,
    );
  ...
  test("missing custom config path is not auto-created", async () => {
    ...
    const code = await main(["--config", missingConfigPath, "remotes"], {
      stdout,
      stderr,
    });
  ...
  test("lists local files with descriptor output", async () => {
    ...
    const stderr = new Capture();
    const code = await main(
      ["--config", configPath, "ls", `file://${tempDir}`],
      { stdout, stderr },
    );
...
`,
    ...
    expect(stdout.value).toContain("source.txt");
    expect(stderr.value).toContain(
      "Warning: Remote 'broken' missing required 'type' field - skipping",
    );
  });
  ...
  test("creates and removes local files from CLI commands", async () => {
    ...
    expect(
      await main(
        ["--config", configPath, "mkdir", `file://${tempDir}`, "created"],
        { stdout, stderr },
      ),
    ).toBe(0);
    ...
    expect(
      await main(
        ["--config", configPath, "rm", `file://${tempDir}`, "delete-me.txt"],
        { stdout, stderr },
      ),
    ).toBe(0);
  ...
  test("returns a non-zero code when rm or mkdir cannot complete", async () => {
    ...
    expect(
      await main(
        ["--config", configPath, "rm", `file://${tempDir}`, "missing.txt"],
        { stdout, stderr },
      ),
    ).toBe(1);
    ...
    expect(
      await main(
        ["--config", configPath, "mkdir", `file://${tempDir}`, "already-there"],
        { stdout, stderr },
      ),
    ).toBe(1);
  ...
  test("uses a bare local path as the browse connection", async () => {
    const stdout = new Capture();
    const stderr = new Capture();
    const code = await main(["--config", configPath, tempDir], {
      stdout,
      stderr,
    });
...
`);
  ...
  test("interactive browse with configured local relative path starts from the process cwd", async () => {
    ...
    expect(browsedPaths).toEqual([await realpath(selectedDir)]);
  });
  ...
  test("reports bad config, missing remotes, unsupported protocols, and usage errors", async () => {
    ...
    expect(
      await main(["--config", badConfigPath, "remotes"], { stdout, stderr }),
    ).toBe(1);
    expect(stderr.value).toContain("Failed to parse TOML configuration");

    stderr.value = "";
    expect(
      await main(["--config", configPath, "ls", "missing"], { stdout, stderr }),
    ).toBe(1);
    expect(stderr.value).toContain("Remote 'missing' not found");

    stderr.value = "";
    expect(
      await main(["--config", configPath, "ls", "gopher://example.com"], {
        stdout,
        stderr,
      }),
    ).toBe(1);
    ...
    expect(
      await main(
        ["--config", configPath, "get", `file://${tempDir}`, "only-remote"],
        { stdout, stderr },
      ),
    ).toBe(1);
    expect(stderr.value).toContain(
