...
import {
  ...
  type AzureDataLakePathItem,
} from "../src/clients/azure_datalake.ts";
...
class FakeAzureDataLakeBackend implements AzureDataLakeBackend {
  ...
  getFileClient(
    path: string,
  ): ReturnType<AzureDataLakeBackend["getFileClient"]> {
    return {
  ...
  getDirectoryClient(
    path: string,
  ): ReturnType<AzureDataLakeBackend["getDirectoryClient"]> {
    return {
      create: async () => {
...
describe("AzureDataLakeClient", () => {
  test("lists paths as file descriptors", async () => {
    const backend = new FakeAzureDataLakeBackend([
      { name: "base/docs", isDirectory: true },
      {
        ...
        contentLength: 42,
        lastModified: new Date("2026-06-21T09:00:00.000Z"),
      },
    ...
    expect(files).toEqual([
      ...
      },
      {
        ...
        type: "file",
        size: 42,
        modifiedTime: new Date("2026-06-21T09:00:00.000Z"),
      },
    ]);
  });
  ...
  test("downloads, uploads, deletes, creates directories, and reports progress", async () => {
    ...
    expect(backend.readCalls).toEqual([
      { path: "remote/source.txt", localPath: localDownload },
    ]);
    expect(backend.uploadCalls).toEqual([
      { path: "remote/upload.txt", localPath: localUpload },
    ]);
