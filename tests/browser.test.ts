...
const state: BrowserState = {
  ...
  entries: [
    { path: "src", name: "src", type: "directory" },
    {
      ...
      type: "file",
      size: 512,
      modifiedTime: new Date("2026-06-20T10:30:00Z"),
    },
  ],
  selected: 0,
  status: "2 items",
...
describe("browser state", () => {
  ...
  test("returns effects for commands that need storage work", () => {
    expect(applyBrowserCommand(state, "open").effect).toEqual({
      type: "open-directory",
      path: "src",
    });
    expect(
      applyBrowserCommand({ ...state, selected: 1 }, "open").state.status,
    ).toBe("Download README.md to local directory? y/n");
    expect(applyBrowserCommand(state, "back").effect).toEqual({ type: "back" });
    expect(applyBrowserCommand(state, "parent").effect).toEqual({
      type: "parent",
    });
  });
  ...
  test("selects entries by case-insensitive prefix", () => {
    ...
    expect(selectedEntry(selectByPrefix(selected, "missing"))?.name).toBe(
      "README.md",
    );
  });
  ...
  test("handles search prompt input without storage effects", () => {
    ...
    const typed = applyBrowserPromptInput(started, {
      type: "text",
      value: "r",
    });
    const submitted = applyBrowserPromptInput(typed.state, { type: "submit" });

    expect(keyToBrowserPromptInput("", { name: "backspace" })).toEqual({
      type: "backspace",
    });
  ...
  test("prompts before deleting files and rejects directories", () => {
    ...
    const confirmed = applyBrowserPromptInput(fileDelete.state, {
      type: "text",
      value: "y",
    });
  ...
  test("prompts before downloading selected files", () => {
    ...
    const confirmed = applyBrowserPromptInput(fileOpen.state, {
      type: "text",
      value: "y",
    });
    const cancelled = applyBrowserPromptInput(fileOpen.state, {
      type: "text",
      value: "n",
    });

    expect(fileOpen.state.status).toBe(
      "Download README.md to local directory? y/n",
    );
    expect(confirmed.effect).toEqual({
      type: "download-file",
      path: "README.md",
  ...
  test("toggles upload mode through effects", () => {
    ...
    const exit = applyBrowserCommand(
      { ...state, mode: "upload" },
      "toggle-upload",
    );
  ...
  test("prompts before uploading selected local files", () => {
    ...
    expect(fileOpen.state.status).toBe(
      "Upload README.md to remote directory? y/n",
    );
    expect(confirmed.effect).toEqual({
      type: "upload-file",
      path: "README.md",
  ...
  test("blocks remote mutation commands in upload mode", () => {
    ...
    expect(applyBrowserCommand(uploadState, "delete").state.status).toBe(
      "Upload mode: select a file or press U to exit",
    );
    expect(applyBrowserCommand(uploadState, "mkdir").state.status).toBe(
      "Upload mode: select a file or press U to exit",
    );
  });
  ...
  test("collects mkdir prompt input into a storage effect", () => {
    ...
    transition = applyBrowserPromptInput(transition.state, {
      type: "text",
      value: "n",
    });
    transition = applyBrowserPromptInput(transition.state, {
  ...
  test("opens expanded help as a prompt and closes it without storage effects", () => {
    ...
    expect(opened.state.prompt).toEqual({
      type: "help",
      previousStatus: "2 items",
    });
...
describe("renderBrowser", () => {
  ...
  test("renders expanded help screen", () => {
    ...
    const rendered = frameToString(frame);
    const uploadHelpState = applyBrowserCommand(
      { ...state, mode: "upload" },
      "help",
    ).state;
    const renderedUploadHelp = frameToString(
      renderBrowserFrame(
        uploadHelpState,
        { width: 90, height: 24 },
        { colors: true },
    ...
    expect(renderedUploadHelp).toContain(
      "\x1b[0;1;38;2;255;255;255;41mKey Commands",
    );
  });

  test("renders confirmation and mkdir prompt dialogs", () => {
    const downloadPrompt = applyBrowserCommand(
      { ...state, selected: 1 },
      "open",
    ).state;
    const deletePrompt = applyBrowserCommand(
      { ...state, selected: 1 },
      "delete",
    ).state;
    let mkdirPrompt = applyBrowserCommand(state, "mkdir").state;
    mkdirPrompt = applyBrowserPromptInput(mkdirPrompt, {
      type: "text",
      value: "n",
    }).state;
    mkdirPrompt = applyBrowserPromptInput(mkdirPrompt, {
    ...
    const renderedDownload = renderBrowser(downloadPrompt, {
      width: 90,
      height: 18,
    });
    const renderedDelete = renderBrowser(deletePrompt, {
      width: 90,
      height: 18,
    });
    ...
    expect(renderedDownload).toContain(
      "Download README.md to local directory?",
    );
    ...
    expect(renderedDownload).toContain("─");
    expect(renderedDownload).toContain("│");
    expect(renderedDelete).toContain(
      "Delete README.md? This cannot be undone.",
    );
  ...
  test("can render ANSI colors for terminal frames", () => {
    const normal = frameToString(
      renderBrowserFrame(state, { width: 80, height: 10 }, { colors: true }),
    );
    const upload = frameToString(
      renderBrowserFrame(
        { ...state, mode: "upload" },
        { width: 80, height: 10 },
        { colors: true },
    ...
    const help = frameToString(
      renderBrowserFrame(
        applyBrowserCommand(state, "help").state,
        { width: 80, height: 10 },
        { colors: true },
      ),
    );
    ...
    expect(help).toContain(
      "\x1b[0;1;38;2;255;255;255;44mpress any key to continue",
    );
  ...
  test("diffs frames line by line after the first render", () => {
    ...
    const second = renderBrowserFrame(
...
describe("runBrowser transfers", () => {
  test("browses protocol-less relative local roots without double-resolving them", async () => {
    ...
    try {
      await waitFor(
        () =>
          output.value.includes("storage.ts") ||
          output.value.includes("Error:"),
        "relative local browser render",
      );
  ...
  test("shows listing errors inside the browser instead of rejecting startup", async () => {
    ...
    await waitFor(
      () => output.value.includes("Connecting..."),
      "initial connecting frame",
    );
    expect(output.value).not.toContain("No files or directories found");
    expect(output.value).not.toContain("Loading...");
    await waitFor(
      () => output.value.includes("Error: list failed"),
      "listing error status",
    );
  ...
  test("redraws the current frame when the terminal is resized", async () => {
    ...
    await waitFor(
      () => output.value.includes("file.txt"),
    ...
    output.emit("resize");
    await waitFor(
      () => output.writes.length > writesBeforeResize,
      "browser resize redraw",
    );
  ...
  test("shows download progress and cancels active transfers before q quits", async () => {
    ...
    const session = {
      ...
      async list() {
        return [
          { path: "big.bin", name: "big.bin", type: "file" as const, size: 10 },
        ];
      },
      async download(
        _remotePath: string,
    ...
    await waitFor(
      () => output.value.includes("big.bin"),
      "initial browser render",
    );
    input.emit("keypress", "\r", { name: "return" });
    await waitFor(
      () => output.value.includes("Download big.bin to local directory? y/n"),
      "download confirmation",
    );
    input.emit("keypress", "y", { name: "y" });
    await waitFor(
      () =>
        downloadStarted && output.value.includes("Transferred: 5 B of 10 B"),
      "download progress",
    );
    ...
    await waitFor(
      () => output.value.includes("Download of big.bin was canceled"),
      "download cancellation status",
    );
