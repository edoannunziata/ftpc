...
describe("remote selector state", () => {
  ...
  test("redacts URL credentials from remote details", () => {
    const entries = remoteEntriesFromConfig(
      parseConfigText(`
[ftp]
type = "ftp"
...
`),
  ...
  test("selects remotes and custom paths", () => {
    ...
    expect(pathPrompt.state.prompt).toEqual({
      type: "path",
      input: "",
      currentPath: "/",
    });
    pathPrompt = applyRemoteSelectorPromptInput(pathPrompt.state, {
      type: "text",
      value: "d",
    });
    pathPrompt = applyRemoteSelectorPromptInput(pathPrompt.state, {
    ...
    expect(selected.effect).toEqual({
      type: "select",
      remote: "alpha",
      path: "/",
    });
    expect(pathPrompt.effect).toEqual({
      type: "select",
      remote: "alpha",
      path: "data",
    });
  ...
  test("uses the default path when path prompt is submitted empty", () => {
    const state = initialRemoteSelectorState(
      remoteEntriesFromConfig(config),
      "/existing",
    );
    const pathPrompt = applyRemoteSelectorCommand(state, "open-path");
    const submitted = applyRemoteSelectorPromptInput(pathPrompt.state, {
      type: "submit",
    });
  ...
  test("searches by prefix and opens details as a dialog prompt", () => {
    ...
    const closed = applyRemoteSelectorPromptInput(details.state, {
      type: "submit",
    });
  ...
  test("handles search prompt input", () => {
    let transition = applyRemoteSelectorCommand(
      initialRemoteSelectorState(remoteEntriesFromConfig(config)),
      "search",
    );
    transition = applyRemoteSelectorPromptInput(transition.state, {
      type: "text",
      value: "z",
    });
    const submitted = applyRemoteSelectorPromptInput(transition.state, {
      type: "submit",
    });
  ...
  test("opens expanded help as a prompt and closes it", () => {
    ...
    const closed = applyRemoteSelectorPromptInput(opened.state, {
      type: "submit",
    });
...
describe("renderRemoteSelectorFrame", () => {
  ...
  test("renders expanded help screen", () => {
    const state = applyRemoteSelectorCommand(
      initialRemoteSelectorState(remoteEntriesFromConfig(config)),
      "help",
    ).state;
  ...
  test("renders details and path dialogs", () => {
    const base = initialRemoteSelectorState(
      remoteEntriesFromConfig(config),
      "/default",
    );
    const details = applyRemoteSelectorCommand(base, "details").state;
    let path = applyRemoteSelectorCommand(base, "open-path").state;
    path = applyRemoteSelectorPromptInput(path, {
      type: "text",
      value: "d",
    }).state;

    const renderedDetails = frameToString(
      renderRemoteSelectorFrame(details, { width: 80, height: 20 }),
    );
    const renderedPath = frameToString(
      renderRemoteSelectorFrame(path, { width: 80, height: 20 }),
    );
...
describe("runRemoteSelector", () => {
  test("redraws the current frame when the terminal is resized", async () => {
    ...
    await waitFor(
      () => output.value.includes("alpha"),
      "initial selector render",
    );
