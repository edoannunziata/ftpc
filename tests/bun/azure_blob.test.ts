...
import { AzureBlobClient, type AzureBlobBackend, type AzureBlobListItem } from "../../src/clients/azure_blob.ts";

class FakeAzureBlobBackend implements AzureBlobBackend {
  objects = new Map<string, string>();
  listCalls: Array<{ delimiter: string; prefix?: string }> = [];
  deleteCalls: string[] = [];
  uploadBlockBlobCalls: Array<{ path: string; body: string; contentLength: number }> = [];

  constructor(private readonly listing: AzureBlobListItem[] = []) {}

  async *listBlobsByHierarchy(delimiter: string, options: { prefix?: string } = {}): AsyncIterable<AzureBlobListItem> {
  ...
  getBlockBlobClient(path: string): ReturnType<AzureBlobBackend["getBlockBlobClient"]> {
    return {
  ...
  async uploadBlockBlob(path: string, body: string, contentLength: number): Promise<void> {
...
describe("AzureBlobClient", () => {
  test("lists blobs as files and blob prefixes as virtual directories", async () => {
    const backend = new FakeAzureBlobBackend([
      ...
      { kind: "blob", name: "base/report.txt", properties: { contentLength: 42, lastModified: new Date("2026-06-21T08:30:00.000Z") } },
      { kind: "blob", name: "base/nested/ignored.txt", properties: { contentLength: 99 } },
    ]);
    ...
    expect(files).toEqual([
      ...
      { path: "report.txt", name: "report.txt", type: "file", size: 42, modifiedTime: new Date("2026-06-21T08:30:00.000Z") },
    ]);
  });
  ...
  test("downloads, uploads, deletes, and creates directory placeholders", async () => {
    ...
    expect(backend.uploadBlockBlobCalls).toEqual([{ path: "remote/new-dir/", body: "", contentLength: 0 }]);
  });
});
