...
import { createDefaultConfig, DEFAULT_CONFIG_TEXT, parseConfigText, listRemotes, getRemote } from "../../src/config.ts";
...
`);
  ...
  test("default config is parseable and includes commented backend examples", async () => {
    ...
    try {
      ...
      expect(text).toContain("SOCKS5 proxy configuration is implemented for FTP, SFTP, and anonymous S3");
    } finally {
