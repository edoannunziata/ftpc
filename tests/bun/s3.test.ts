...
import { createUnsignedS3Backend, S3Client, type S3Backend, type S3ListResponse } from "../../src/clients/s3.ts";

class FakeS3Backend implements S3Backend {
  objects = new Map<string, Uint8Array>();
  listCalls: Array<{ prefix?: string; delimiter?: string; continuationToken?: string }> = [];
  deleteCalls: string[] = [];

  constructor(private readonly listResponses: S3ListResponse[] = []) {}

  async list(input?: { prefix?: string; delimiter?: string; continuationToken?: string } | null): Promise<S3ListResponse> {
  ...
  async write(path: string, data: Parameters<S3Backend["write"]>[1]): Promise<number> {
...
function httpResponse(body: string, headers: Record<string, string> = {}): Buffer {
...
<ListBucketResult>
  ...
  <Contents>
    ...
    const response = await backend.list({ prefix: "base/", delimiter: "/", continuationToken: "old-token" });
    ...
    expect(response).toEqual({
      ...
      contents: [{ key: "base/report&notes.txt", size: 42, lastModified: "2026-06-20T10:30:00.000Z" }],
      isTruncated: true,
  ...
  test("uses unsigned REST requests for anonymous transfers", async () => {
    ...
    expect(requests).toEqual([
      { url: "https://storage.example.com/root/public-bucket/folder/file%20name.txt", method: "GET", body: undefined },
      { url: "https://storage.example.com/root/public-bucket/folder/upload.txt", method: "PUT", body: "upload body" },
      { url: "https://storage.example.com/root/public-bucket/folder/remove.txt", method: "DELETE", body: undefined },
    ]);
  });
  ...
  test("defaults to unsigned REST backend when credentials are not configured", async () => {
    ...
    const client = new S3Client({
      ...
      fetch: async (input) => {
        requests.push(input.toString());
        return new Response("<ListBucketResult><Contents><Key>file.txt</Key><Size>5</Size></Contents></ListBucketResult>");
      },
    });

    const files = await client.list("/");

    expect(files).toEqual([{ path: "file.txt", name: "file.txt", type: "file", size: 5, modifiedTime: undefined }]);
    expect(requests).toEqual(["https://storage.example.com/public-bucket?list-type=2&delimiter=%2F"]);
  });
  ...
  test("routes unsigned anonymous listings through SOCKS5 proxy transport", async () => {
    ...
    const backend = createUnsignedS3Backend({
      ...
      proxyConnector: async (options) => {
        ...
        const socket = new ScriptedHttpSocket(chunkedHttpResponse(`
<ListBucketResult>
  <Contents><Key>file.txt</Key><Size>7</Size></Contents>
    ...
    expect(request.startsWith("GET /public-bucket?list-type=2&delimiter=%2F HTTP/1.1\r\n")).toBe(true);
  ...
  test("routes unsigned anonymous uploads through SOCKS5 proxy transport", async () => {
    ...
    const request = sockets[0].requestText();
    expect(request.startsWith("PUT /public-bucket/folder/upload.txt HTTP/1.1\r\n")).toBe(true);
    ...
    expect(emptyRequest.startsWith("PUT /public-bucket/folder/empty/ HTTP/1.1\r\n")).toBe(true);
  ...
  test("lists objects as files and common prefixes as virtual directories", async () => {
    const backend = new FakeS3Backend([
      {
        ...
        contents: [
          { key: "base/", size: 0 },
          { key: "base/report.txt", size: 42, lastModified: "2026-06-20T10:30:00.000Z" },
    ...
    expect(backend.listCalls).toEqual([{ prefix: "base/", delimiter: "/", continuationToken: undefined }]);
    expect(files).toEqual([
      { path: "photos", name: "photos", type: "directory", size: 0 },
      { path: "report.txt", name: "report.txt", type: "file", size: 42, modifiedTime: new Date("2026-06-20T10:30:00.000Z") },
    ]);
  });
  ...
  test("downloads, uploads, deletes, and creates directory placeholders", async () => {
    const backend = new FakeS3Backend();
    backend.objects.set("remote/source.txt", new TextEncoder().encode("from s3"));
    backend.objects.set("remote/delete.txt", new TextEncoder().encode("delete me"));
    const client = new S3Client({ bucketName: "bucket", backend });
    ...
    expect(new TextDecoder().decode(backend.objects.get("remote/upload.txt"))).toBe("to s3");
    expect(deleted).toBe(true);
    expect(missingDelete).toBe(false);
    expect(madeDirectory).toBe(true);
    expect(new TextDecoder().decode(backend.objects.get("remote/new-dir/"))).toBe("");
