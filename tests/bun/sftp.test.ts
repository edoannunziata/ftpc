...
import { SftpClient, expandHomePath, type SftpBackend } from "../../src/clients/sftp.ts";
import { ListingError, TransferError } from "../../src/errors.ts";

function stats(options: { directory?: boolean; size?: number; mtime?: number }): Stats {
  return {
    mode: 0,
    uid: 0,
    gid: 0,
...
describe("SftpClient", () => {
  test("connects lazily and maps directory listings", async () => {
    ...
    expect(files).toEqual([
      { path: "docs", name: "docs", type: "directory", size: undefined, modifiedTime: new Date(1780000000 * 1000) },
      { path: "readme.txt", name: "readme.txt", type: "file", size: 123, modifiedTime: new Date(1780000001 * 1000) },
    ]);
  });
  ...
  test("downloads, uploads, deletes, creates directories, and closes", async () => {
    ...
    expect(progress).toEqual([{ bytes: 9, total: 9 }, { bytes: 7, total: 7 }]);
    expect(backend.fastGetCalls).toEqual([{ remotePath: "/remote/source.txt", localPath: localDownload }]);
    expect(backend.fastPutCalls).toEqual([{ localPath: localUpload, remotePath: "/remote/upload.txt" }]);
    expect(backend.unlinkCalls).toEqual(["/remote/delete.txt", "/remote/missing.txt"]);
  ...
  test("wraps lazy connection failures in operation errors", async () => {
    ...
    expect(downloadError.message).toBe("Failed to download '/remote/source.txt' from SFTP host 'sftp.example.com': connect failed");
    expect(uploadError).toBeInstanceOf(TransferError);
    expect(uploadError.message).toBe(`Failed to upload '${localUpload}' to SFTP host 'sftp.example.com': connect failed`);
  });
  ...
  test("closes the SFTP session when an in-flight transfer is aborted", async () => {
    ...
    class HangingSftpBackend extends FakeSftpBackend {
      override async fastGet(remotePath: string, localPath: string): Promise<void> {
        this.fastGetCalls.push({ remotePath, localPath });
        startedTransfer();
