...
function resourceNameFromStorageUrl(url: string, label: string, protocol?: "azure" | "blob"): string {
  const parsed = protocol === undefined
    ? parseStorageUrl(url)
    : parseStorageUrl(url.includes("://") ? url : `${protocol}://${url}`);
  const [resourceName] = parsed.path.split("/").filter((part) => part !== "");
  if (resourceName === undefined) {
    throw new Error(`${label} URL must include the remote resource name in the first path segment`);
  }
  return resourceName;
}
...
function ftpConnection(envName: string, tls: boolean): IntegrationConnection {
  ...
  return configConnection(remoteName, `[${remoteName}]
type = "ftp"
url = ${tomlString(url)}
tls = ${tls ? "true" : "false"}
...
function sftpConnection(): IntegrationConnection {
  ...
  if (effectivePassword !== undefined || keyFilename !== undefined) {
    return configConnection("sftp-it", `[sftp-it]
type = "sftp"
url = ${tomlString(url)}
...
function s3Connection(): IntegrationConnection {
  ...
  return configConnection("s3-it", `[s3-it]
type = "s3"
url = ${tomlString(url)}
...
return configConnection("azure-it", `[azure-it]
type = "azure"
url = ${tomlString(url)}
...
return configConnection("blob-it", `[blob-it]
type = "blob"
url = ${tomlString(url)}
...
async function assertTransferWorkflow(session: StorageSession, label: string): Promise<void> {
  ...
  try {
    ...
    expect(listed.some((entry) => entry.name === fileName && entry.type === "file")).toBe(true);
...
function defineIntegrationTest(label: string, connectionFactory: () => IntegrationConnection): void {
  ...
  runner(`${label} real service transfer workflow`, async () => {
    const session = Storage.connect(connection.connection, { config: connection.config });
    await assertTransferWorkflow(session, label.toLocaleLowerCase().replace(/[^a-z0-9]+/g, "-"));
...
describe("integration connection helpers", () => {
  ...
  test("SFTP helper lets explicit environment credentials override URL credentials", () => {
    withEnv({
      FTPC_INTEGRATION_SFTP_URL: "url-user:url-pass@sftp.example.com/home/user",
  ...
  test("Azure helper derives filesystem from no-scheme account URLs", () => {
    withEnv({
      FTPC_INTEGRATION_AZURE_URL: "account.dfs.core.windows.net/filesystem/base",
  ...
  test("Blob helper derives container from no-scheme account URLs", () => {
    withEnv({
      FTPC_INTEGRATION_BLOB_URL: "account.blob.core.windows.net/container/base",
  ...
  test("Azure and Blob helpers keep explicit resource environment overrides", () => {
    withEnv({
      ...
      const lakeRemote = azureDataLakeConnection().config?.remotes.get("azure-it");
      const blobRemote = azureBlobConnection().config?.remotes.get("blob-it");

      expect(lakeRemote).toMatchObject({ type: "azure", filesystem: "env-filesystem" });
      expect(blobRemote).toMatchObject({ type: "blob", container: "env-container" });
    });
  });
});

describe("real service integrations", () => {
  defineIntegrationTest("FTP", () => ftpConnection("FTPC_INTEGRATION_FTP_URL", false));
  defineIntegrationTest("FTPS", () => ftpConnection("FTPC_INTEGRATION_FTPS_URL", true));
