...
class ScriptedSocket extends Duplex {
  ...
  constructor(
    private readonly onWrite: (chunk: Buffer, socket: ScriptedSocket) => void,
  ) {
    super();
  }
...
describe("connectSocks5", () => {
  test("connects through a no-auth SOCKS5 proxy", async () => {
    ...
    const socket = new ScriptedSocket((chunk, proxy) => {
      ...
      expect([...chunk.subarray(0, 5)]).toEqual([
        0x05,
        0x01,
        0x00,
        0x03,
        "sftp.example.com".length,
      ]);
      requestedHost = chunk
        .subarray(5, 5 + "sftp.example.com".length)
        .toString("utf8");
      requestedPort = chunk.readUInt16BE(5 + "sftp.example.com".length);
      proxy.send(
        Buffer.from([0x05, 0x00, 0x00, 0x01, 127, 0, 0, 1, 0x12, 0x34]),
      );
    });
  ...
  test("authenticates with username and password", async () => {
    ...
    const socket = new ScriptedSocket((chunk, proxy) => {
      ...
      expect([...chunk.subarray(0, 5)]).toEqual([
        ...
        0x00,
        0x03,
        "server.internal".length,
      ]);
      expect(
        chunk.subarray(5, 5 + "server.internal".length).toString("utf8"),
